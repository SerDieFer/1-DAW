...
UPDATE CLIENTES
   SET ciudad = 'Alicante'
 WHERE limite_credito < ALL (SELECT SUM(precio_venta)
                               FROM PRODUCTOS
...
-- 17. Actualiza la ciudad a Madrid para aquellos clientes que tengan un límite de crédito mensual inferior a ALGUNO 
-- de los precios de los productos de la categoría Ornamentales.
 ...
 UPDATE CLIENTES
  ...
  WHERE codCliente = (SELECT TOP(1) pe.codCliente
                        ...
                        WHERE pe.codPedido = dp.codPedido
                          AND dp.codProducto = pro.codProducto 
                          AND UPPER(pro.refInterna) = 'OR-179'
...
INSERT INTO HISTORICO_CLIENTES
     SELECT *,
            GETDATE()
       FROM CLIENTES
      WHERE UPPER(nombre_cliente) LIKE('%S%')
...
UPDATE cli
   SET cli.pais = hc.pais,
       cli.codPostal = hc.codPostal
  FROM CLIENTES cli INNER JOIN HISTORICO_CLIENTES hc
    ON cli.codCliente = hc.codCliente
